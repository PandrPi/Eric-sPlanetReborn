Shader "Hidden/Custom/Atmosphere"
{
	HLSLINCLUDE
...
#include "Packages/com.unity.postprocessing/PostProcessing/Shaders/StdLib.hlsl"
	...
	uniform float4 scatteringCoefficients;
	...
	float3 calculateLight2(float3 rayOrigin, float3 rayDir, float rayLength)
	{
		...
		for (int i = 0; i < numInScatteringPoints; i++)
		{
			...
			inScatteredLight += localDensity * stepSize * (1.0 - sunRayLength / (atmosphereRadius * 2));
			//inScatteredLight += localDensity * transmittance * stepSize;
			inScatterPoint += rayDir * stepSize;
		}
		...
		return saturate(inScatteredLight);
	}
